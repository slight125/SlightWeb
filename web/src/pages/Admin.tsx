...
export default function Admin() {
  ...
  const [form, setForm] = useState({ name: '', price: '', condition: 'new', specs: '' });
  ...
  const createItem = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setForm({ name: '', price: '', condition: 'new', specs: '' });
    }
  };
  ...
  return (
    <div className="min-h-dvh bg-neutral-950 text-neutral-100 px-4 py-8 max-w-5xl mx-auto">
      ...
      <form onSubmit={createItem} className="mt-6 grid md:grid-cols-5 gap-2">
        ...
        {items.map(p => (
          <div key={p.id} className="rounded border border-neutral-800 bg-neutral-900/60 p-3 space-y-2">
            ...
            <div className="flex gap-3 text-sm">
              <button className="text-brand-400" onClick={()=>update(p.id, { name: p.name, price: Number(p.price), condition: p.condition, specs: p.specs })}>Save</button>
